...
### Важно: Сетевой псевдоним (Alias)

Скрипт генерации конфигураций (`generate-configs.py`) определяет имя upstream-сервиса как первую часть доменного имени, указанного в переменной `DOMAINS` (например, `my-app` из `my-app.example.com`).

Чтобы `nginx-proxy` мог разрешить это имя во внутренний IP-адрес вашего сервиса через Docker DNS, контейнер вашего сервиса должен быть подключен к общей сети (`nginx-proxy_network`) с **сетевым псевдонимом (alias)**, совпадающим с этим именем.

Пример для сервиса `my-app`, доступного по `my-app.example.com`:
...
services:
  my-app:
    # ... другие настройки ...
    networks:
      default: # Ваша внутренняя сеть
      nginx-proxy_network: # Общая сеть с nginx-proxy
        aliases:
          - my-app # Псевдоним = первая часть домена
```

Убедитесь, что вы добавили соответствующий `alias` в `docker-compose.yml` каждого сервиса, который вы проксируете через этот `nginx-proxy`.
